...
class ASPP(nn.Module):
    def __init__(self, in_channel, depth):
        ...
        self.conv1 = nn.Conv2d(in_channel, depth, kernel_size=1, stride=1, padding=0)
        self.context1 = nn.Sequential(
            nn.Conv2d(depth, depth, 3, 1, padding=1, dilation=1),
            nn.BatchNorm2d(depth),
            nn.ReLU(inplace=True),
            nn.Conv2d(depth, depth, 3, 1, padding=2, dilation=2),
            nn.BatchNorm2d(depth),
            nn.ReLU(inplace=True),
            nn.Conv2d(depth, depth, 3, 1, padding=3, dilation=3),
            nn.BatchNorm2d(depth),
            nn.ReLU(inplace=True),
        )
        self.context2 = nn.Sequential(
            ...
            nn.BatchNorm2d(depth),
            nn.ReLU(inplace=True),
            nn.Conv2d(depth, depth, 3, 1, padding=5, dilation=5),
            nn.BatchNorm2d(depth),
            nn.ReLU(inplace=True),
        )
        self.context3 = nn.Sequential(
            ...
            nn.BatchNorm2d(depth),
            nn.ReLU(inplace=True),
            nn.Conv2d(depth, depth, 3, 1, padding=9, dilation=9),
            nn.BatchNorm2d(depth),
            nn.ReLU(inplace=True),
        )
        self.output = nn.Conv2d(depth * 3, depth, 1, 1)
    def forward(self, x):
        img = self.conv1(x)
        img = self.output(torch.cat([self.context1(img),self.context2(img),self.context3(img)],dim=1))
        return img
...
class SFAFMA(nn.Module):
    def __init__(self, n_class):
        ...
        self.conv0 = nn.Conv2d(192, 96, 1, 1)
        self.conv1 = nn.Conv2d(768, 384, 1, 1)
        self.conv2 = nn.Conv2d(1536, 768, 1, 1)
        self.conv3 = nn.Conv2d(4224, 2112, 1, 1)
        self.conv4 = nn.Conv2d(4416, 2208, 1, 1)
        ...
        self.local_att0 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(int(channels[0] // 4), channels[0], kernel_size=1, stride=1, padding=0),
        ...
        self.local_att1 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(int(channels[1] // 4), channels[1], kernel_size=1, stride=1, padding=0),
        ...
        self.local_att2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(int(channels[2] // 4), channels[2], kernel_size=1, stride=1, padding=0),
        ...
        self.local_att3 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(int(channels[3] // 4), channels[3], kernel_size=1, stride=1, padding=0),
        ...
        self.local_att4 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(int(channels[4] // 4), channels[4], kernel_size=1, stride=1, padding=0),
